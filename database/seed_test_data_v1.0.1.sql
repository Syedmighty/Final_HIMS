-- ============================================================================
-- HIMS - PRODUCTION-READY SEED TEST DATA v1.0.1
-- ============================================================================
-- Description: Corrected sample data that properly triggers all database events
-- Usage: Run after v1_initial.sql or after applying v1.0_to_v1.0.1_migration.sql
-- IMPORTANT: This data is for TESTING ONLY - replace with real data in production
-- ============================================================================
...
-- ============================================================================
-- CLEANUP FOR RERUN SAFETY
-- ============================================================================

-- Clear transactional data (preserves schema and master data)
...
DELETE FROM wastage_returns;
DELETE FROM issue_line_items;
DELETE FROM issues;
...
-- Clear master data (for clean rerun)
...
DELETE FROM sqlite_sequence WHERE name IN (
  ...
  'wastage_returns', 'stock_transfers', 'stock_transfer_items',
  'recipes', 'recipe_ingredients', 'stock_alerts', 'stock_adjustments'
);
...
INSERT INTO users (uuid, username, password_hash, full_name, email, phone, created_by) VALUES
  ('user-admin-001', 'admin', '$2b$10$uCye.CyUZ8F52A5ONsZ8eOJv7sYfWk3P0H0dEr.WHi3o.Z8/fEIXq', 'System Administrator', 'admin@hotel.com', '+91-9876543210', 'system'),
  ('user-manager-001', 'store_manager', '$2b$10$uCye.CyUZ8F52A5ONsZ8eOJv7sYfWk3P0H0dEr.WHi3o.Z8/fEIXq', 'Rajesh Kumar', 'rajesh@hotel.com', '+91-9876543211', 'user-admin-001'),
  ('user-chef-001', 'chef_indian', '$2b$10$uCye.CyUZ8F52A5ONsZ8eOJv7sYfWk3P0H0dEr.WHi3o.Z8/fEIXq', 'Ramesh Singh', 'ramesh@hotel.com', '+91-9876543212', 'user-admin-001'),
  ('user-chef-002', 'chef_chinese', '$2b$10$uCye.CyUZ8F52A5ONsZ8eOJv7sYfWk3P0H0dEr.WHi3o.Z8/fEIXq', 'Wei Zhang', 'wei@hotel.com', '+91-9876543213', 'user-admin-001'),
  ('user-accountant-001', 'accountant', '$2b$10$uCye.CyUZ8F52A5ONsZ8eOJv7sYfWk3P0H0dEr.WHi3o.Z8/fEIXq', 'Priya Sharma', 'priya@hotel.com', '+91-9876543214', 'user-admin-001'),
  ('user-inactive-001', 'old_user', '$2b$10$uCye.CyUZ8F52A5ONsZ8eOJv7sYfWk3P0H0dEr.WHi3o.Z8/fEIXq', 'Inactive Test User', 'old@hotel.com', '+91-9876543215', 'user-admin-001');

-- Mark one user as inactive for testing
UPDATE users SET is_active = 0, deleted_at = datetime('now'), deleted_by = 'user-admin-001'
WHERE uuid = 'user-inactive-001';
...
-- ============================================================================
-- 4. UNIT CONVERSIONS (FIXED: Corrected conversion factors)
-- ============================================================================
...
-- Bottle/Can conversions (FIXED: 330ml can = 0.33L, not 330L!)
INSERT INTO unit_conversions (product_uuid, from_unit_id, to_unit_id, conversion_factor, created_by) VALUES
  ('prod-bev-001', 9, 3, 1, 'user-manager-001'),      -- 1 bottle = 1 litre
  ('prod-bev-002', 10, 4, 0.33, 'user-manager-001'),  -- FIXED: 1 can = 0.33 litre (was 330)
  ('prod-bev-003', 9, 3, 1, 'user-manager-001');      -- 1 bottle = 1 litre

-- ============================================================================
-- 5. PURCHASES (FIXED: Insert as draft, then update to trigger events)
-- ============================================================================

-- Purchase 1: Vegetables (Insert as draft first)
INSERT INTO purchases (
  ...
  'INV-VEG-2025-001', '2025-11-01', 'draft', 'user-manager-001', NULL
);
...
INSERT INTO purchase_line_items (purchase_id, product_uuid, quantity, unit_id, unit_price, tax_rate, tax_amount, line_total) VALUES
  (1, 'prod-veg-001', 100, 1, 40, 5, 200, 4200),
  (1, 'prod-veg-002', 50, 1, 30, 5, 75, 1575),
  (1, 'prod-veg-003', 150, 1, 25, 5, 187.5, 3937.5),
  (1, 'prod-veg-004', 10, 1, 80, 5, 40, 840);

-- Purchase 2: Meat & Poultry
INSERT INTO purchases (
  uuid, purchase_number, supplier_id, location_id, purchase_date,
  invoice_number, invoice_date, status, created_by, approved_by
) VALUES (
  'purchase-002', 'PO-2025-002', 2, 1, '2025-11-02',
  'INV-MEAT-2025-001', '2025-11-02', 'draft', 'user-manager-001', NULL
);

INSERT INTO purchase_line_items (purchase_id, product_uuid, quantity, unit_id, unit_price, tax_rate, tax_amount, line_total) VALUES
  (2, 'prod-meat-001', 50, 1, 180, 0, 0, 9000),
  (2, 'prod-meat-002', 20, 5, 250, 0, 0, 5000),
  (2, 'prod-meat-003', 30, 1, 450, 0, 0, 13500),
  (2, 'prod-meat-004', 15, 1, 300, 5, 225, 4725),
  (2, 'prod-meat-005', 10, 1, 600, 5, 300, 6300);
...
INSERT INTO purchases (
  ...
  'INV-DAIRY-2025-001', '2025-11-03', 'draft', 'user-manager-001', NULL
);

INSERT INTO purchase_line_items (purchase_id, product_uuid, quantity, unit_id, unit_price, tax_rate, tax_amount, line_total) VALUES
  (3, 'prod-dairy-001', 100, 3, 60, 0, 0, 6000),
  (3, 'prod-dairy-002', 20, 1, 450, 12, 1080, 10080),
  (3, 'prod-dairy-003', 10, 1, 500, 12, 600, 5600),
  (3, 'prod-dairy-004', 25, 3, 150, 12, 450, 4200),
  (3, 'prod-dairy-005', 15, 1, 280, 5, 210, 4410);

-- TRIGGER EVENT: Approve purchases to populate stock_levels
UPDATE purchases
SET status = 'received', approved_by = 'user-admin-001', approved_at = datetime('now')
WHERE id IN (1, 2, 3);
...
-- ============================================================================
-- 7. ISSUES TO KITCHENS (FIXED: Insert as pending, then update)
-- ============================================================================
...
INSERT INTO issues (
  ...
  'user-chef-001', 'user-manager-001', 'pending', 'user-chef-001', NULL
);
...
INSERT INTO issues (
  ...
  'user-chef-002', 'user-manager-001', 'pending', 'user-chef-002', NULL
);
...
-- TRIGGER EVENT: Approve issues to update stock
UPDATE issues
SET status = 'issued', approved_by = 'user-admin-001', approved_at = datetime('now')
WHERE id IN (1, 2);
...
-- ============================================================================
-- 10. STOCK TRANSFER (FIXED: Insert as pending, then update)
-- ============================================================================
...
INSERT INTO stock_transfers (
  ...
  'user-manager-001', 'pending', 'user-manager-001', NULL
);
...
-- TRIGGER EVENT: Complete transfer to update stock
UPDATE stock_transfers
SET status = 'completed', approved_by = 'user-admin-001', approved_at = datetime('now')
WHERE id = 1;
...
UNION ALL
SELECT 'Stock Alerts', COUNT(*) FROM stock_alerts;

-- Show current stock levels (should be populated by triggers)
SELECT '========== CURRENT STOCK LEVELS ==========' AS status;
...
SELECT '========== ACTIVE ALERTS ==========' AS status;
...
SELECT '========== STOCK ADJUSTMENTS SUMMARY ==========' AS status;
